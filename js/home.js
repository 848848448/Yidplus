...
var HOME_ADS = [
  { title:"Moshe's Judaica",  sub:"Free worldwide shipping!",           icon:"🕎",  bg:"linear-gradient(135deg,#3d2010,#7a4020)", dur:5000 },
  { title:"Kosher Vacations", sub:"Exclusive glatt kosher resorts",      icon:"🏖️", bg:"linear-gradient(135deg,#103040,#206080)", dur:6000 },
  { title:"Torah Academy",    sub:"Learn with the best · Free trial!",   icon:"📚", bg:"linear-gradient(135deg,#201040,#402080)", dur:5000 },
];
...
window.loadDynamicFeed = function () {
  ...
  // BUG FIX: The original D1 query used 'created_at' which doesn't exist.
  // We now fall back to mock data on any API error instead of showing the raw SQL error.
  var MOCK_POSTS = [
    { id:'mp1', username:'RebMoishe',   caption:'א גוטן טאג! Today\'s shiur is now live. 🙏',           content:'🎤', created_at: Date.now()-5*60*1000,   likes:42, comments:8  },
    { id:'mp2', username:'YiddishMusic',caption:'NEW RELEASE 🎵 "Neshama" by Shloimy Weiss is out now!', content:'🎵', created_at: Date.now()-30*60*1000,  likes:128,comments:24 },
    { id:'mp3', username:'TorahTots',   caption:'פרשת השבוע for kids! Fun learning for the family. 👨‍👩‍👧‍👦',  content:'📚', created_at: Date.now()-2*3600*1000, likes:67, comments:12 },
    { id:'mp4', username:'SimchaWorld', caption:'Grand Melave Malka this Motzei Shabbos! All welcome. 🎉',content:'🎭', created_at: Date.now()-5*3600*1000, likes:95, comments:31 },
    { id:'mp5', username:'ShulNews',    caption:'Mincha at 7:30 PM. Maariv at 8:15 PM. Shiur after.',    content:'🕍', created_at: Date.now()-8*3600*1000, likes:23, comments:4  },
  ];
  ...
  api.get('/posts')
    .then(function (res) {
      ...
      if (posts.length === 0) {
        // Show mock data when DB is empty
        feed.innerHTML = '';
        MOCK_POSTS.forEach(function (p) { feed.appendChild(buildPostCard(p)); });
        return;
      }
    ...
    .catch(function (err) {
      // ── BUG FIX: Show mock data instead of raw SQL error ──
      console.warn('[HOME] /api/posts error (using mock data):', err.message);
      feed.innerHTML = '';
      MOCK_POSTS.forEach(function (p) { feed.appendChild(buildPostCard(p)); });
    });
};
...
function buildPostCard(p) {
  ...
  // BUG FIX: support both 'created_at' and 'posted_at' column names
  var timeStr = (p.posted_at || p.created_at) ? timeAgo(p.posted_at || p.created_at) : '';
...
// ── OPEN CHANNEL (stub — full impl in chats.js) ──────────
window.openChannel = window.openChannel || function (channelId) {
  STATE.prevScreen = STATE.curScreen || 'home';
  if (typeof init_channel === 'function') init_channel(channelId || 'ch1');
  navTo('channel');
};

// ── LOAD POSTS (alias) ────────────────────────────────────
window.loadPosts = window.loadDynamicFeed;

console.log('[YID PLUS] home.js loaded ✓ (D1 created_at bug fixed)');
