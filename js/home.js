...
// Uses Supabase realtime for broadcasts
// ============================================================
...
async function listenBroadcasts() {
  ...
  broadcastChannel = supabase
    ...
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'broadcasts' },
      payload => showBroadcast(payload.new.text)
    )
    .subscribe();
}
...
function showBroadcast(text) {
  ...
  bar.innerHTML = `<div style="margin:.5rem .75rem;background:rgba(201,168,76,.08);border:.5px solid var(--border2);border-radius:10px;padding:.65rem 1rem;display:flex;align-items:flex-start;gap:.6rem">
    ...
    <span style="font-size:.8rem;color:var(--gold-l);line-height:1.4;flex:1">${text}</span>
...
function buildStatusRow() {
  ...
  STATUSES.forEach((s, i) => {
    ...
    el.onclick   = () => openSV(i);
    el.innerHTML = `<div class="status-ring${s.viewed?' viewed':''}"><div class="status-inner">${s.emoji}</div></div>
...
function buildAds() {
  ...
  ADS.forEach((ad, i) => {
    ...
    s.innerHTML = `<div style="font-size:2.5rem">${ad.icon}</div>
...
function runAd() {
  ...
  function tick(now) {
    ...
    if (pct < 100) { adRaf = requestAnimationFrame(tick); }
    else {
...
function buildShortsPrev() {
  ...
  .forEach(s => {
    ...
    c.className = 'short-prev-card'; c.onclick = () => navTo('shorts');
...
function buildChannelsPrev() {
  ...
  .forEach(c => {
    ...
    card.className = 'ch-preview-card'; card.onclick = () => openChannel(c.n);
...
/* ── STATUS VIEWER ── */
function openSV(i) {
  svUser = STATUSES[i]; svSlide = 0; STATUSES[i].viewed = true;
  document.querySelectorAll('#status-row .status-ring')[i+1]?.classList.add('viewed');
...
function showSVSlide(idx) {
  ...
  const sl=svUser.slides[idx], el=document.getElementById('sv-slide'), bars=document.getElementById('sv-bars');
  if (!el||!bars) return;
  bars.innerHTML = svUser.slides.map((_,j) =>
    `<div class="sv-bar"><div class="sv-bar-fill ${j<idx?'done':j===idx?'running':''}"></div></div>`).join('');
  el.style.opacity='0'; el.style.background=sl.bg||'#111'; el.style.color=sl.color||'#fff';
